...
const RecipePreview = ({ recipe }: { recipe: RecipePreviewType }) => {
  ...
  const imageUrl =
    recipe.images[0]?.url || "/recipe-images/recipe-placeholder.jpg";
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="flex flex-col items-center p-2 bg-muted/30 rounded-lg">
              ...
              <span className="font-medium">
                {recipe.prepTime.duration} {recipe.prepTime.timeUnit}
              </span>
            </div>
            <div className="flex flex-col items-center p-2 bg-muted/30 rounded-lg">
              <span className="text-muted-foreground text-sm">Cook Time</span>
              <span className="font-medium">
                {recipe.cookTime.duration} {recipe.cookTime.timeUnit}
              </span>
            </div>
          </div>
        </CardContent>
